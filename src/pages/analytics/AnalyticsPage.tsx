      ...
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="w-full max-w-xl rounded-2xl border border-red-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Check that Django is running and that
            Snowflake is reachable.
          </p>
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          subtitle={`${formatNumber(overview.closed_claims)} closed`}
          icon={FileCheck2}
        />

        <KpiCard
          title="Estimated Loss"
          value={formatCurrency(overview.total_estimated_loss)}
          subtitle={`Average ${formatCurrency(overview.avg_estimated_loss)}`}
          icon={CircleDollarSign}
        />

        <KpiCard
          title="Approved Amount"
          value={formatCurrency(overview.total_approved_amount)}
          subtitle={`Average ${formatCurrency(overview.avg_approved_amount)}`}
          icon={CheckCircle2}
        />

        <KpiCard
          title="Settlement Amount"
          value={formatCurrency(overview.total_settlement_amount)}
          subtitle={`${formatNumber(overview.settled_claims)} settled`}
          icon={WalletCards}
        />
      </section>
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          value={formatPercentage(overview.settlement_rate)}
        ...
        <KpiCard
          ...
          value={formatNumber(overview.ai_review_required_claims)}
        ...
        <KpiCard
          title="Closed Claims"
          value={formatNumber(overview.closed_claims)}
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        >
          {data.monthly_trend.length > 0 ? (
            <ClaimsMonthlyTrendChart data={data.monthly_trend} />
          ) : (
        ...
        >
          {data.claims_by_status.length > 0 ? (
            ...
            <EmptyState message="No claim status data available." />
          )}
        </SectionCard>
      </section>
      ...
      >
        <StatusTable data={data.claims_by_status} />
      </SectionCard>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        >
          {data.claims_by_type.length > 0 ? (
            ...
            <EmptyState message="No claim type data available." />
          )}
        </SectionCard>
        ...
        >
          {data.claims_by_policy_type.length > 0 ? (
            <ClaimsByPolicyTypeChart
              data={data.claims_by_policy_type}
            />
          ) : (
            <EmptyState message="No policy type data available." />
          )}
        </SectionCard>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        >
          {data.ai.length > 0 ? (
            <HumanReviewChart data={data.ai} />
          ) : (
            <EmptyState message="No AI analytics available." />
          )}
        </SectionCard>
      ...
      >
        <CustomerTable data={data.customer_claims} />
      </SectionCard>

      <footer className="py-4 text-center text-xs text-slate-400">
        Analytics powered by Django REST API and Snowflake.
      </footer>
    </div>
  );
}
      ...
      <table className="min-w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-500">
            ...
            <th className="px-3 py-3 text-right">Estimated Loss</th>
          ...
          {data.map((item) => (
            ...
            >
              <td className="px-3 py-3">
                <StatusBadge status={item.claim_status} />
              </td>

              <td className="px-3 py-3 text-right font-medium text-slate-800">
                {formatNumber(item.total_claims)}
              </td>

              <td className="px-3 py-3 text-right text-slate-600">
                {formatCurrency(item.total_estimated_loss)}
              </td>

              <td className="px-3 py-3 text-right text-slate-600">
                {formatCurrency(item.total_approved_amount)}
              </td>

              <td className="px-3 py-3 text-right text-slate-600">
                {formatCurrency(item.total_settlement_amount)}
              </td>
            </tr>
          ))}
        </tbody>
...
function SettlementTable({
  ...
  if (data.length === 0) {
    return <EmptyState message="No settlement data available." />;
  }
      ...
      <table className="min-w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-500">
            ...
            <th className="px-3 py-3 text-right">Claims</th>
            <th className="px-3 py-3 text-right">Settled</th>
            <th className="px-3 py-3 text-right">Settlement</th>
        ...
        <tbody>
          {data.map((item) => (
            ...
            >
              ...
              <td className="px-3 py-3 text-right text-slate-600">
                {formatNumber(item.settled_financial_records)}
              </td>
              ...
              <td className="px-3 py-3 text-right text-slate-600">
                {formatPercentage(item.settlement_to_estimated_ratio)}
              </td>
            </tr>
          ))}
        </tbody>
...
function ProcessingTable({
  ...
  if (data.length === 0) {
    return (
      <EmptyState message="No processing analytics available." />
    );
  }
      ...
      <table className="min-w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-500">
            ...
            <th className="px-3 py-3 text-right">≤ 3 Days</th>
            <th className="px-3 py-3 text-right">≤ 7 Days</th>
          </tr>
        </thead>
        ...
        <tbody>
          {data.map((item) => (
            ...
            >
              ...
              <td className="px-3 py-3 text-right">
                {formatDays(item.avg_processing_days)}
              </td>

              <td className="px-3 py-3 text-right">
                {formatDays(item.min_processing_days)}
              </td>

              <td className="px-3 py-3 text-right">
                {formatDays(item.max_processing_days)}
              </td>

              <td className="px-3 py-3 text-right">
                {formatNumber(item.claims_processed_within_3_days)}
              </td>

              <td className="px-3 py-3 text-right">
                {formatNumber(item.claims_processed_within_7_days)}
              </td>
            </tr>
          ))}
        </tbody>
...
function CustomerTable({
  ...
  if (data.length === 0) {
    return (
      <EmptyState message="No customer claim data available." />
    );
  }
      ...
      <table className="min-w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-500">
            ...
            <th className="px-3 py-3 text-right">Approved</th>
            <th className="px-3 py-3 text-right">Settled</th>
            <th className="px-3 py-3 text-right">AI Review</th>
          </tr>
        </thead>
        ...
        <tbody>
          {data.map((customer) => (
            ...
            >
              ...
              <td className="px-3 py-3 text-right">
                {formatNumber(customer.total_claims)}
              </td>

              <td className="px-3 py-3 text-right">
                {formatCurrency(customer.total_estimated_loss)}
              </td>

              <td className="px-3 py-3 text-right">
                {formatCurrency(customer.total_approved_amount)}
              </td>

              <td className="px-3 py-3 text-right">
                {formatCurrency(customer.total_settlement_amount)}
              </td>

              <td className="px-3 py-3 text-right">
                {customer.ai_review_required_claims > 0 ? (
